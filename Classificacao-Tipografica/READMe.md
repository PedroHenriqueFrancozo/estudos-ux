# 🔡 Classificação Tipográfica
A classificação serve para escolher a fonte certa com base na sua estrutura visual e no objetivo da interface.

### 1. Características Visuais (O Desenho)
As fontes são classificadas principalmente pela presença ou ausência de serifas e pela forma de seus traços:

- **Serifadas (Serif):** Possuem pequenos traços nas extremidades. Transmitem tradição, autoridade e seriedade. (Ex: Times New Roman, Garamond).
- **Sem Serifas (Sans Serif):** Têm traços retos e limpos. Transmitem modernidade, tecnologia e minimalismo. (Ex: Helvetica, Roboto).
- **Cursivas (Script):** Imitam a escrita à mão. Transmitem elegância ou pessoalidade.
- **Decorativas (Display):** Estilos únicos para títulos, com muita personalidade, mas baixa legibilidade em textos longos.

### 2. Leitura Clara (Legibilidade)
Para garantir uma leitura fluida, o design foca na distinguibilidade:

- **Contraste:** A diferença entre traços grossos e finos. Contraste moderado ajuda o olho a seguir a linha sem cansar.
- **Abertura (Aperture):** O espaço aberto em letras como 'c', 'e' e 's'. Aberturas maiores evitam que as letras pareçam borrões em tamanhos pequenos.
- **Altura-x:** Fontes com altura-x maior facilitam a leitura em telas digitais.

### 3. Tipografia "Modesta" (Invisível)
No UX, muitas vezes buscamos uma tipografia modesta.

- **O que é:** É aquela fonte que não chama atenção para si mesma. Ela é tão funcional e equilibrada que o usuário foca apenas no conteúdo e não no desenho da letra.
- **Onde usar:** No corpo de textos, artigos e formulários. A fonte deve ser "transparente": o usuário lê sem perceber que está lendo.